            ...
            try {
                loop@ while (isActive) {
                    ...
                    if (data.startsWithBytes(POLICY_FILE_REQUEST)) {
                        Fancam.debug(Tags.Socket) { "[SOCKET] Policy file requested and sent" }
                        connection.write(POLICY_FILE_RESPONSE)
                        return@launch
                    }

                    // ignore 0x00 byte flush after policy request (expected)
                    if (data.startsWithBytes(byteArrayOf(0x00)) && data.drop(1).isEmpty()) {
                        continue
                    }
